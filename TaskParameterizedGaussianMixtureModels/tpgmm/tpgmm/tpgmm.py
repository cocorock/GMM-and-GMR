...
class TPGMM(ClassificationModule):
    ...
    def _update_h(self, probabilities: ndarray) -> ndarray:
        ...
        # for i in range(17):
        #     print(f"{i}")
        #     print(f"denominator: {denominator[i]}")
        #     # print(f"numerator: {numerator[i].T}")

        result = numerator / denominator

        if np.isnan(result).any() or np.isinf(result).any():
                print('h contains NaN or Inf values. Cannot proceed with fitting.')
                print(f"numerator shape: {numerator.shape}")
                print(f"denominator shape: {denominator.shape}")
                print(f'sum of numerator: {np.sum(numerator, axis=1)}\n')
                print(f'sum of denominator: {np.sum(denominator)}\n')
                print(f'Number of NaNs: {np.isnan(result).sum()}\n')
                print(f'Number of Infs: {np.isinf(result).sum()}\n')
    ...
    def fit(self, X: ndarray) -> None:
        ...
        # print(f"sum of probabilities:{np.sum(probabilities, axis=2)}")
        # print(f"self.weights_: {self.weights_}\n")
        
        log_likelihood = self._log_likelihood(probabilities)
        # print(f"\t Initial log likelihood: {log_likelihood}\n")

        if np.isnan(probabilities).any() or np.isinf(probabilities).any() or np.isneginf(probabilities).any(): 
            print('Data contains NaN or Inf values. Cannot proceed with fitting.')
            print(f'Number of NaNs: {np.isnan(probabilities).sum()}\n')
            print(f'Number of Infs: {np.isinf(probabilities).sum()}\n')
            print(f'Number of Infs: {np.isneginf(probabilities).sum()}\n')
            raise ValueError("probabilities contains NaN or Inf values")
        
        for epoch_idx in range(self._max_iter):
            # print(f"Epoch {epoch_idx + 1}/{self._max_iter}")

            # Expectation
            h = self._update_h(probabilities)
            # print(f"shape of h:{h.shape}")
            if np.isnan(h).any() or np.isinf(h).any() or np.isneginf(h).any():
                print('h contains NaN or Inf values. Cannot proceed with fitting.')
                print(f'Number of NaNs: {np.isnan(h).sum()}\n')
                print(f'Number of Infs: {np.isinf(h).sum()}\n')
                print(f'Number of -Infs: {np.isneginf(h).sum()}\n')
            ...
            # print(f"shape of probabilities:{probabilities.shape}")
            if np.isnan(probabilities).any() or np.isinf(probabilities).any() or np.isneginf(probabilities).any():
                print('probabilities contains NaN or Inf values. Cannot proceed with fitting.')
                print(f'Number of NaNs: {np.isnan(probabilities).sum()}\n')
                print(f'Number of Infs: {np.isinf(probabilities).sum()}\n')
                print(f'Number of -Infs: {np.isneginf(probabilities).sum()}\n')
                raise ValueError("probabilities contains NaN or Inf values")
            ...
            if np.isnan(updated_log_likelihood).any() or np.isinf(updated_log_likelihood).any() or np.isneginf(updated_log_likelihood).any():
                print('updated_log_likelihood contains NaN or Inf values. Cannot proceed with fitting.')
                print(f'Number of NaNs: {np.isnan(updated_log_likelihood).sum()}\n')
                print(f'Number of Infs: {np.isinf(updated_log_likelihood).sum()}\n')
                print(f'Number of -Infs: {np.isneginf(updated_log_likelihood).sum()}\n')
                raise ValueError("updated_log_likelihood contains NaN or Inf values")
